...
if ( ! function_exists( 'ctp_switch' ) ) {
	...
	function ctp_switch() {
		...
		if ( ! check_ajax_referer( 'catch_breadcrumb_plugin_tabs_nonce', 'catch_breadcrumb_plugin_tabs_security', false ) ) {
		...
		$option_value[ $option_name ] = absint( $value );

		if ( update_option( 'ctp_options', $option_value ) ) {
			echo esc_html( $value );
		} else {
			esc_html_e( 'Connection Error. Please try again.', 'catch-breadcrumb' );
		}
